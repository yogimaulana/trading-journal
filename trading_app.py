...
if not st.session_state.logged_in:
    ...
    with col_center:
        ...
        st.markdown("<p style='text-align: center; color: #AAAAAA;'>Platform jurnal profesional dengan sistem verifikasi email & keamanan modern.</p>", unsafe_allow_html=True)
        
        st.markdown("---")
        st.markdown("<p style='text-align: center; font-weight: bold; margin-bottom: 5px;'>📱 Atur Mode Tampilan Layar Anda:</p>", unsafe_allow_html=True)
        
        selected_mode = st.radio(
            "Pilih Mode Layar",
            ["🖥️ Mode Desktop (Lebar)", "📱 Mode Kompak (HP / Mobile)"],
            index=0 if st.session_state.view_mode == "🖥️ Mode Desktop (Lebar)" else 1,
            horizontal=True,
        ...
        auth_tab1, auth_tab2, auth_tab3 = st.tabs(["🔑 Masuk (Login)", "📝 Daftar Akun (Email)", "🔄 Ubah / Lupa Password"])
        
        with auth_tab1:
            st.markdown("<br>", unsafe_allow_html=True)
            l_user = st.text_input("Username", key="l_user", placeholder="Masukkan username Anda...")
            l_pass = st.text_input("Password", type="password", key="l_pass", placeholder="Masukkan password Anda...")
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        with auth_tab2:
            st.markdown("<br>", unsafe_allow_html=True)
            r_user = st.text_input("Buat Username Baru", key="r_user", placeholder="Pilih username unik...")
            r_email = st.text_input("Email Aktif", key="r_email", placeholder="nama@email.com")
            r_pass = st.text_input("Buat Password Baru", type="password", key="r_pass", placeholder="Buat password aman...")
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("✨ Daftar Akun Sekarang"):
                if r_user.strip() == "" or r_email.strip() == "" or r_pass.strip() == "":
                    ...
                    if success:
                        st.success(f"🎉 {msg} Silakan pindah ke tab 'Masuk (Login)'.")
                    else:
                        st.error(f"⚠️ {msg}")
                        ...
                        if datetime.now() <= datetime.strptime(row[1], '%Y-%m-%d %H:%M:%S'):
                            if new_p1 == new_p2 and len(new_p1) > 0:
                                if is_same_as_old_password(f_user, new_p1):
                                    st.error("⚠️ Password baru tidak boleh sama dengan password lama Anda!")
                                else:
                                    conn = sqlite3.connect('trading_journal.db')
                                    c = conn.cursor()
                                ...
                                st.error("⚠️ Password baru tidak cocok atau kosong.")
                        else:
                            st.error("⚠️ Kode verifikasi sudah kedaluwarsa. Silakan minta kode baru.")
                    else:
